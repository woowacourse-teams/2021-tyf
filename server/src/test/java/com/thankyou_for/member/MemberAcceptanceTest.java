package com.thankyou_for.member;

import com.thankyou_for.AcceptanceTest;
...
import com.thankyou_for.member.dto.*;
import com.thankyou_for.member.exception.*;
import com.thankyou_for.payment.domain.Item;
import com.thankyou_for.admin.AdminAcceptanceTest;
import com.thankyou_for.donation.DonationAcceptanceTest;
import com.thankyou_for.payment.PaymentAcceptanceTest;
...
import static com.thankyou_for.auth.AuthAcceptanceTest.회원가입_후_로그인되어_있음;
import static com.thankyou_for.auth.AuthAcceptanceTest.회원생성을_요청;
...
public class MemberAcceptanceTest extends AcceptanceTest {
    ...
    public void curations() {
        ...
        String adminToken = AdminAcceptanceTest.관리자_로그인("test-id", "test-password").getToken();
        List<RequestingAccountResponse> requestingAccountResponses = AdminAcceptanceTest.요청_계좌_목록_조회(adminToken).body().jsonPath().getList(".", RequestingAccountResponse.class);
        requestingAccountResponses.forEach(it -> AdminAcceptanceTest.요청_계좌_승인(it.getMemberId(), adminToken));

        SignUpResponse signUpResponse = PaymentAcceptanceTest.충전완료_된_사용자("donator@gmail.com", "KAKAO", "donator", "pagename");
        DonationAcceptanceTest.후원_생성("pagename4", 1000L, signUpResponse.getToken());
        DonationAcceptanceTest.후원_생성("pagename3", 2000L, signUpResponse.getToken());
    ...
    public void detailedPoint() {
        //given
        SignUpResponse signUpResponse = PaymentAcceptanceTest.충전완료_된_사용자("donator@gmail.com", "KAKAO", "donator", "donator");

        SignUpResponse creatorSignUpResponse = 회원가입_후_로그인되어_있음("creator@gmail.com", "KAKAO", "creator", "creator");
        DonationAcceptanceTest.후원_생성("creator", 10000L, signUpResponse.getToken()).as(DonationResponse.class);
    ...
    public void getAccountInfoAfterAccountRegistered() {
        ...
        String token = AdminAcceptanceTest.관리자_로그인("test-id", "test-password").getToken();
        AdminAcceptanceTest.요청_계좌_승인(1L, token);
    ...
    public void requestExchangeAmountLessThanLimit() {
        ...
        String token = AdminAcceptanceTest.관리자_로그인("test-id", "test-password").getToken();
        AdminAcceptanceTest.요청_계좌_승인(1L, token);
        PaymentAcceptanceTest.페이먼트_생성(Item.ITEM_1.name(), signUpResponse.getToken());
        DonationAcceptanceTest.후원_생성("pagename", 9999L, signUpResponse.getToken());
