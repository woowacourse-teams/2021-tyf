...
import com.thankyou_for.payment.exception.IllegalPaymentInfoException;
import com.thankyou_for.payment.exception.PaymentAlreadyCancelledException;
import com.thankyou_for.payment.exception.RefundVerificationBlockedException;
...
public class Payment extends BaseTimeEntity {
    ...
    private void validatePaymentCancel(PaymentInfo paymentInfo) {
        if (paymentInfo.getStatus() != PaymentStatus.CANCELLED) {
            updateStatus(paymentInfo.getStatus());
            throw IllegalPaymentInfoException.from(IllegalPaymentInfoException.ERROR_CODE_NOT_CANCELLED, paymentInfo.getModule());
        }
    ...
    private void validatePaymentInfo(PaymentInfo paymentInfo) {
        if (!merchantUid.equals(paymentInfo.getMerchantUid())) {
            ...
            throw IllegalPaymentInfoException.from(IllegalPaymentInfoException.ERROR_CODE_INVALID_MERCHANT_UID, paymentInfo.getModule());
        }

        if (!amount.equals(paymentInfo.getAmount())) {
            updateStatus(PaymentStatus.INVALID);
            throw IllegalPaymentInfoException.from(IllegalPaymentInfoException.ERROR_CODE_INVALID_AMOUNT, paymentInfo.getModule());
        }

        if (!itemName.equals(paymentInfo.getItemName())) {
            updateStatus(PaymentStatus.INVALID);
            throw IllegalPaymentInfoException.from(IllegalPaymentInfoException.ERROR_CODE_INVALID_CREATOR, paymentInfo.getModule());
        }
    }
